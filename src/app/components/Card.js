...
export default function Card({path, className = ''}) {
    ...
    return <Link href={`/produk/${path}`} className={className}>
        <div className="relative flex flex-col rounded-xl shadow-xl group/card overflow-hidden max-w-sm">
            ...
            <div className="flex flex-col gap-2 p-4 h-28">
                ...
                <p className="line-clamp-2 font-semibold text-gray-500 text-ellipsis">{data.description}</p>
            </div>
            <div className="flex items-center pt-2 border-2 border-transparent border-t-gray-400 group-hover/card:text-gray-700 text-gray-600 m-4 p-t-2">
