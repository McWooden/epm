...
export default function CardLarge({path, className = ''}) {
    ...
    if (!data) return <div>{path} tidak ditemukan</div>
        return <Link href={`/produk/${path}`} className={className}>
            <div className={`h-72 rounded-md overflow-hidden bg-cover bg-center`} style={{backgroundImage: `url(${data.image[0]})`}}>
            <div className="bg-gray-900 bg-opacity-50 flex items-center h-full">
                <div className="px-10 max-w-xl">
                    ...
                    <div href="https://wa.me/6281329295252" className="flex items-center mt-4 px-3 py-2 bg-blue-600 text-white text-sm uppercase font-medium rounded hover:bg-blue-500 focus:outline-none focus:bg-blue-500 w-fit">
